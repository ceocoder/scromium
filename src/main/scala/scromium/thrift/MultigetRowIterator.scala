...
abstract class MultigetRowIterator[C <: Columnar](map : JMap[ByteBuffer,JList[ColumnOrSuperColumn]]) extends RowIterator[C] {
...
class MGColumnRowIterator(map : JMap[ByteBuffer,JList[ColumnOrSuperColumn]]) extends MultigetRowIterator[Column](map) {
  def unpackColumn(c : ColumnOrSuperColumn) : Column = {
    Thrift.unpackColumn(c)
  }
}

class MGSuperColumnRowIterator(map : JMap[ByteBuffer,JList[ColumnOrSuperColumn]]) extends MultigetRowIterator[SuperColumn](map) {
